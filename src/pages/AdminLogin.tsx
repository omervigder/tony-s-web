...
export default function AdminLogin() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // AuthContext verifies admin status via Firestore.
      // If not admin it will sign the user out and set accessDenied=true.
    } catch (err: any) {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#FFF5F7] flex items-center justify-center p-4">
      <div className="bg-white border border-[#fce4ec] rounded-2xl p-8 w-full max-w-sm shadow-sm">
        ...
        {/* Error banner */}
        {displayError && (
          <div className="mb-5 flex items-start gap-2 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-red-500 text-sm">
        ...
        >
          ...
          {loading ? 'מתחבר...' : 'Sign in with Google'}
        </button>

        <p className="text-center text-gray-400 text-xs mt-6">
          גישה מוגבלת למנהלים מורשים בלבד
        </p>
      </div>
    </div>
  );
